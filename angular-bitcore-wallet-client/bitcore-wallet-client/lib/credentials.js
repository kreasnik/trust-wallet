...
Credentials.create = function (network, account, cb) {
	...
	x._expand(function (error) {
		console.log("WalletJniPort--error =  " + error);
		cb(error);
		return x;
	});
};
...
Credentials.createWithMnemonic = function (network, passphrase, language, account, cb) {
	_checkNetwork(network);
	...
	;
	console.log("WalletJniPort--createWithMnemonic start");
	var x = new Credentials();

	x.network = network;
	x.account = account;
	x.xPrivKey = "xprv9s21ZrQH143K2iujAViMorZT6chJdwGL15AVvYB5FJEhF7hQkHwzjmNSd79s6qVGDHKdbKAWRgAETK5YBsDmxMHX9zRnLfJc4ZF6wD774EA";//m.toHDPrivateKey(passphrase, network).toString();
	x.mnemonic = m.phrase;
	x.mnemonicHasPassphrase = !!passphrase;
	console.log("WalletJniPort--createWithMnemonic end");
	console.log("credentials: createWithMnemonic " + network + ', ' + passphrase + ',' + language + ',' + account);
	x._expand(function (result) {
		//console.log("WalletJniPort: createWithMnemonic cb(error)");
		cb(result);
	});
	return x;
};

Credentials.fromExtendedPrivateKey = function (xPrivKey, account, cb) {
	var x = new Credentials();
	x.xPrivKey = xPrivKey;
	x.account = account || 0;
	x._expand(function (result) {
 ...
 */
Credentials.fromExtendedPublicKey = function (xPubKey, source, entropySourceHex, account, derivationStrategy, cb) {
	...
	console.log("WalletJniPort--fromExtendedPublicKey pubkey =  " + xPubKey.toString());
	var x = new Credentials();
	x.xPubKey = this.xPubKey;
...
Credentials.prototype._expand = function (cb) {
	...
	if (this.xPrivKey) {
		...
		console.log('WalletJniPort--_expand xPubKey: ' + this.xPubKey);
	} else {
	}
	//xuejin create mnemonic in se
	var derivePath = this.getBaseAddressDerivationPath();
	console.log("WalletJniPort--derive derivePath = " + derivePath);
	cordova.plugins.walletjniport.derive(
		"12345678",
		derivePath,
		0,
		0,
		...
		function onSuccess(result) {
			console.log("WalletJniPort--se result.pubkey =  " + result.pubkey.toString());
			//this.xPubKey = result.pubkey.toString();
			//console.log("WalletJniPort--se this xPubKey = " + this.xPubKey.toString());
			// this.publicKeyRing = [{
		...
		function onError(error) {
			console.log("WalletJniPort--se xPubKey error " + error);
			alert('derive ' + error);
		}
	);
	this.publicKeyRing = [{

 ...
 */
API.prototype.seedFromRandom = function (opts, cb) {
	...
	opts = opts || {};
	this.credentials = Credentials.create(opts.network || 'livenet', opts.account || 0, function (error) {
		cb(error);
	});
};
 ...
 */
API.prototype.seedFromRandomWithMnemonic = function (opts, cb) {
	// $.checkArgument(arguments.length <= 1, 'DEPRECATED: only 1 argument accepted.');
	// $.checkArgument(_.isUndefined(opts) || _.isObject(opts), 'DEPRECATED: argument should be an options object.');

	opts = opts || {};
	//console.log("WalletJniPort: seedFromRandomWithMnemonic " + JSON.stringify(opts));
	this.credentials = Credentials.createWithMnemonic(opts.network || 'livenet', opts.passphrase, opts.language || 'en', opts.account || 0, function (result) {
		//console.log("WalletJniPort: seedFromRandomWithMnemonic result : " + JSON.stringify(result));
		cb(result);
	});
};
 ...
 */
API.prototype.seedFromExtendedPrivateKey = function (xPrivKey, account, cb) {
	console.log("WalletJniPort: seedFromExtendedPrivateKey xPrivKey = " + xPrivKey);
	this.credentials = Credentials.fromExtendedPrivateKey(xPrivKey, account, function (result) {
		cb(result);
	});
};
 ...
 * @param {String} opts.derivationStrategy - default 'BIP44'
 */
API.prototype.seedFromExtendedPublicKey = function (xPubKey, source, entropySourceHex, opts, cb) {
	//$.checkArgument(_.isUndefined(opts) || _.isObject(opts));

	opts = opts || {};
	console.log("WalletJniPort: seedFromExtendedPublicKey " + JSON.stringify(opts));
	this.credentials = Credentials.fromExtendedPublicKey(xPubKey, source, entropySourceHex, opts.account || 0, opts.derivationStrategy || Constants.DERIVATION_STRATEGIES.BIP44, function (result) {
		cb(result);
	});
};
...
API.prototype.isComplete = function () {
	return true;//this.credentials && this.credentials.isComplete();
};
...
API.prototype.createWallet = function (walletName, m, n, opts, cb) {
	...
	//console.log("WalletJniPort -- createWalletByDevices self.credentials = " + JSON.stringify(self.credentials));
	if (!self.credentials) {
		//console.log("WalletJniPort -- createWalletByDevices !credentials");
		...
		self.seedFromRandom({
			...
			//console.log("WalletJniPort -- createWalletByDevices xPubkey = " + self.credentials.xPubKey);
			cb(error);
		});
	} else {
	...
	walletDefinedByKeys.createWalletByDevices(self.credentials.xPubKey, opts.account || 0, m, opts.cosigners || [], walletName, function (wallet) {
		...
		console.log("WalletJniPort -- self.credentials.walletId: " + self.credentials.walletId);
...
API.prototype.createAddress = function (is_change, cb) {
	...
	console.log('WalletJniPort -- createAddress wallet=' + this.credentials.walletId + ', is_change=' + is_change);
...
API.prototype.sendMultiPayment = function (opts, cb) {
	...
	var sigtemp;
	console.log("xuejin: sendMultiPayment start !!!");
	opts.signWithLocalPrivateKey = function (wallet_id, account, is_change, address_index, text_to_sign, handleSig) {
		...
		console.log("xuejin: sendMultiPayment text_to_sign:" + JSON.stringify(text_to_sign));

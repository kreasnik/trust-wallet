...
function addWallet(wallet, xPubKey, account, arrWalletDefinitionTemplate, onDone){
	...
	console.log("WalletJniPort -- addWallet!!");
	async.series([
		function(cb){
			console.log("WalletJniPort -- cb 111 !!");
			...
			setTimeout(function(){
				console.log("WalletJniPort -- onDone 111 !!");
				onDone();
			}, 1000);
		},
		function(cb){
			console.log("WalletJniPort -- cb 222 !!");
			async.eachSeries(
				arrDeviceAddresses,
				function(device_address, cb2){
					console.log("WalletJniPort -- adding device "+device_address+' to wallet '+wallet);
		...
		function(cb){
			...
			console.log("WalletJniPort -- cb 333 !!");
			async.eachSeries(
				arrSigningPaths,
		...
		console.log("WalletJniPort -- addWallet done : " + wallet + ", arrDeviceAddresses.length : " + arrDeviceAddresses.length );
...
function createWalletByDevices(xPubKey, account, count_required_signatures, arrOtherDeviceAddresses, walletName, handleWallet){
	console.log('WalletJniPort -- createWalletByDevices: xPubKey='+xPubKey+", account="+account);
		...
		db.query(
			...
			function(rows){
				...
				rows.forEach(function(row){
					...
					console.log('WalletJniPort -- pubkey for wallet '+wallet+' path '+path+' device '+row.device_address+' xpub '+row.extended_pubkey+': '+params['pubkey@'+row.device_address]);
				});

...
angular.module('copayApp.services')
  .factory('addressService', function(profileService, $log, $timeout, lodash, gettextCatalog) {
    ...
    root._createAddress = function(walletId, cb) {
      ...
      //console.log('WalletJniPort -- _createAddress walletId:'+walletId);

      client.createAddress(0, function(err, addr) {
        if (err)
            throw "impossible err creating address";
		  //console.log('WalletJniPort -- _createAddress cb addr.address:'+addr.address);
    ...
    root.getAddress = function(walletId, forceNew, cb) {
        if (forceNew) {
            ...
            var client = profileService.getClient(walletId);
            client.getAddresses({reverse: false, limit: 1, is_change: 0}, function(err, addr) {
                if (err)
                    return cb(err);
                if (addr.length > 0)

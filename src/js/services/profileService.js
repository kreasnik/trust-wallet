...
angular.module('copayApp.services').factory('profileService', function profileServiceFactory($rootScope, $location, $timeout, $filter, $log, lodash, storageService, bwcService, configService, pushNotificationsService, isCordova, gettext, gettextCatalog, nodeWebkit, uxLanguage) {
    ...
    root.bindProfile = function(profile, cb) {
		...
		console.log('WalletJniPort --- bindProfile : '+JSON.stringify(root.profile));
	...
	root.loadAndBindProfile = function (cb) {
		...
		console.log('WalletJniPort --- loadAndBindProfile');
		...
		storageService.getDisclaimerFlag(function (err, val) {
			if (!val) {
				...
				storageService.getProfile(function (err, profile) {
					console.log('WalletJniPort --- bindProfile : '+JSON.stringify(profile));
					if (err) {
	...
	root._seCreateWallet = function(opts, cb){
		var lang = uxLanguage.getCurrentLanguage();
		console.log("WalletJniPort-- _seCreateWallet for language "+lang);
		if(opts.noWallet == "restore"){
			cordova.plugins.walletjniport.recoveryMnemonic(
				opts.passphrase,
				lang,
				12,
				function onSuccess(result) {
					console.log("WalletJniPort--se recovery mnemonic success");
					cb(opts, result);
				},
				function onError(error) {
					console.log("WalletJniPort--se recovery mnemonic error " + error);
					alert('recoveryMnemonic ' + error);
					cb(opts, error);
				}
			);
		}else {
			cordova.plugins.walletjniport.createMnemonic(
				opts.passphrase,
				lang,
				12,
				function onSuccess(result) {
					console.log("WalletJniPort--se create mnemonic success");
					cb(opts, result);
				},
				function onError(error) {
					console.log("WalletJniPort--se create mnemonic error " + error);
					alert('createMnemonic ' + error);
					cb(opts, error);
				}
			);
		}
    ...
    root._seedWallet = function(opts, cb) {
          ...
          //
        	...
        	// }
      	// } else
		console.log("WalletJniPort-- opts = " + JSON.stringify(opts));
		if (opts.extendedPrivateKey) {
        	try {
				console.log("WalletJniPort-- opts.extendedPrivateKey");
          		walletClient.seedFromExtendedPrivateKey(opts.extendedPrivateKey, opts.account || 0, function (result) {
					return cb(result, walletClient);
				});
        	} catch (ex) {
          		$log.warn(ex);
        	...
        	try {
				console.log("WalletJniPort-- opts.extendedPublicKey");
			...
			this._seCreateWallet(opts, function (opts, result) {
				try {
    ...
    root._createNewProfile = function(opts, cb) {
		...
		console.log("WalletJniPort_createNewProfile");
        root._seedWallet(opts, function(result, walletClient) {
			console.log("WalletJniPort -- _createNewProfile + result : " + JSON.stringify(result));
            // if (err)
            //     return cb(err);
			var config = configService.getSync();
			...
			var sepubkey = result.pubkey;
			//root.focusedClient.credentials.xPubKey = sepubkey;
			var cred = JSON.parse(walletClient.export());
			console.log("WalletJniPort -- _createNewProfile + before cred ====== " + JSON.stringify(cred));
			cred.xPubKey = sepubkey;
			cred.publicKeyRing = [{
				xPubKey: sepubkey,
			}];
			walletClient.import(JSON.stringify(cred));
			//console.log("WalletJniPort -- createWallet + after cred ====== " + JSON.stringify(cred));
			walletClient.createWallet(walletName, 1, 1, {
				...
				var device = require('trustnote-common/device.js');
				var tempDeviceKey = device.genPrivKey();
				console.log("WalletJniPort -- add Profile");
	...
	root.createWallet = function (opts, cb) {
		$log.debug('WalletJniPort -- Creating Wallet:', opts);  // Creating Wallet: {"m":1,"n":1,"name":"wwwddd","networkName":"livenet","cosigners":[]}
		...
		walletDefinedByKeys.readNextAccount(function (account) {
			console.log("WalletJniPort -- next account = " + account);
			...
			root._seedWallet(opts, function (result, walletClient) {
				...
				var sepubkey = result.pubkey;
				var cred = JSON.parse(walletClient.export());
				console.log("WalletJniPort -- createWallet + before cred ====== " + JSON.stringify(cred));
				cred.xPubKey = sepubkey;
				cred.publicKeyRing = [{
					xPubKey: sepubkey,
				}];
    ...
    root.create = function(opts, cb) {
      	...
      	configService.get(function(err) {
        	root._createNewProfile(opts, function(result, p) {
          		//if (err) return cb(err);
